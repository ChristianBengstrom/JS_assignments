...
let Umo = {
    ...
    move() {
        if (this.x + this.dx > this.canvas.getWidth()-this.r
                || this.x + this.dx < 0+this.r)
              this.dx = -this.dx;
        if (this.y + this.dy > this.canvas.getHeight()-this.r
                || this.y + this.dy < 0+this.r)
              this.dy = -this.dy;

        this.x += this.dx;
        this.y = (Math.pow(this.x - 200, 2) / 150)+50;
    },
